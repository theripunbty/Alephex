...
const Hero: React.FC = () => {
  ...
  const [amountIn] = useState(''); // Placeholder amount for demonstration
  const [tokenIn] = useState('ETH'); // Default token
  ...
  const handleSwap = async () => {
    ...
    try {
      console.log("Calling swapExactTokenForToken...");
      const amountInValue = BigInt(amountIn).toString();
      const amountOutMin = '0'; // Set minimum output amount directly as string
      const deadline = Math.floor(Date.now() / 1000) + 1200; // 20 minutes from now

      console.log(
        `Calling swapExactTokenForToken: ` +
        `tokenPair=${tokenPair}, ` +
        `sender=${sender}, ` +
        `tokenIn=${tokenIn}, ` +
          ...
          <div className="bg-gradient-to-br from-gray-800 to-gray-900/80 backdrop-blur-lg shadow-lg rounded-xl p-6 mx-auto w-full max-w-sm">
            ...
            <div className="flex flex-col gap-4">
              ...
              <button
                ...
                onClick={handleSwap} // Attach the handleSwap function here
              >
                Swap Tokens
              </button>
            </div>
